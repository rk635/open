...
I like hot chocolate with whipped cream a lot.
...
1. Improve Experience
  * Goal: 56 years of experience
  * Currently: perhaps 1
2. Eat Pizza
  * Eat every kind of pizza in the world
  * Ever
...
<!--- Fill you answer here. --->
In my major we have a lot of problems and we try to solve them. [Math](https://www.math.cornell.edu/m/)
...
|:------- |:------:| :---:|-----:|
|Walrus|Bacon|Kafka on the Shore|ALternative Library|
|Pikachu|Water|Samurai's Garden|Mallot|
|Doggo|Pizza|Magic School Bus|Tree behind Dickson|
...
$$ \int_{\partial \omega} \phi = \int_{\omega} \textbf{d}\phi $$
...
diam = .4 * u.m
flow = 2
temp = u.Quantity(np.arange(273,473,10), u.kelvin)
visc = pc.viscosity_kinematic(temp)

reynolds = pc.re_pipe(flow, diam, visc)

plt.plot(temp, reynolds, label = 'Reynolds Number')
plt.xlabel('Temperature (Kelvin)')
plt.ylabel('Reynolds Number (Units)')
...
plt.legend(loc = 'lower right', ncol = 1)
plt.tight_layout()
plt.savefig('/Users/Ryan/Documents/reyPlot.png')
plt.show()
```

![Plot](/Images/reyPlot.png)·
...
have a fun february break! -medhavi
